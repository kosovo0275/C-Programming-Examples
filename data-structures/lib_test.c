...
int test_linked_list() {
	...
	test_msg_start("Test Linked List End - One Node Only");
		...
		if(test_node1 == NULL) 
			result++;
		if(list_tail(test_list1) != test_node1) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Removing Node From List With More Than One Node");
		...
		if(test_list1->pNext != test_node2) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Removing Node From Empty List");
		...
		if(list_rm_node(test_list1, test_node1) != -1) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Inserting Node After");
		...
		/* test beginning */
		if(test_list1->pNext != test_node1) 
			result++;
		/*...and end nodes. */
		if(list_tail(test_list1) != test_node2) 
			result++;
		/* end node next should be null */
		if(list_tail(test_list1)->pNext != NULL) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Test Lengths");
		...
		if(list_len(test_list1) != 1) 
			result++;

		list_ins_tail(test_list1);
		if(list_len(test_list1) != 2) 
			result++;

		list_ins_before(test_list1, list_tail(test_list1));
		if(list_len(test_list1) != 3) 
			result++;

		list_ins_after(test_list1, list_tail(test_list1));
		if(list_len(test_list1) != 4) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Previous Node Check");
		...
		test_node1 = NULL;
		test_node2 = NULL;
		if(list_prev_node(test_list1, NULL) != NULL) 
			result++;
		if(list_prev_node(NULL, test_list1->pNext) != NULL) 
			result++;
		if(list_prev_node(NULL, test_node1 + 1000) != NULL) 
			result++;
		test_node1 = list_ins_head(test_list1);
		if(list_prev_node(test_list1, test_list1->pNext) != NULL) 
			result++;
		test_node2 = list_ins_head(test_list1);
		if(list_prev_node(test_list1, test_node1) != test_node2) 
			result++;
		if(list_prev_node(test_list1, test_node2) != NULL) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Removing Nodes By Address");
		...
		if(list_rm_node(NULL, test_list1->pNext)!= -1) 
			result++;
		if(list_rm_node(test_list1, NULL) != -1) 
			result++;
		test_node1 = list_ins_head(test_list1);
		test_node2 = list_ins_head(test_list1);
		...
		if(list_len(test_list1) != 0) 
			result++;
		if(list_rm_node(test_list1, list_tail(test_list1)) != -1) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Removing Nodes After Address");
		...
		if(list_rm_next(test_list1, test_node1) != 1) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Removing Nodes Before Address");
		...
		test_node1 = NULL;
		test_node2 = NULL;
		if(list_rm_before(test_list1, NULL) != -1) 
			result++;
		if(list_rm_before(NULL, NULL) != -1) 
			result++;
		if(list_rm_before(test_list1 + 1000, NULL) != -1) 
			result++;
		test_node1 = list_ins_tail(test_list1);
		test_node2 = list_ins_tail(test_list1);
		if(list_rm_before(test_list1, test_node2 + 1000) != -1) 
			result++;
		if(list_rm_before(test_list1, test_node2) != 1) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Copying List");
		...
		test_node2 = list_ins_tail(test_list1);
		if(list_copy(test_list2, test_list1) != 0) 
			result++;
		if(test_list1->pNext == test_list2->pNext) 
			result++;
		if(test_list1->count != 2) 
			result++;
		if(test_list2->count != 2) 
			result++;
		if(test_list1->pNext->pData != test_list2->pNext->pData) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Copying Empty List");
		...
		if(list_copy(test_list2, test_list1) != 0) 
			result++;
		if(test_list1->pNext != NULL) 
			result++;
		if(test_list2->pNext != NULL) 
			result++;
		if(test_list1->count != 0) 
			result++;
		if(test_list2->count != 0) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Swap Nodes In List");
		...
		if(list_node_swap(test_node1, test_node2) != 0) 
			result++;
		if(test_node1->pNext != pTrack[2]) 
			result++;
		if(test_node2->pNext != pTrack[1]) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - List Next Preprocessor");
		...
		test_node1 = list_ins_head(test_list1);
		test_node2 = list_ins_head(test_list1);
		if(list_next(test_node1) == test_node2) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Append Lists - Pointer Tracking");
		...
		if(test_list1->count != 7) 
			result++;
		if(!list_search(test_list1, pTrack[0])) 
			result++;
		if(!list_search(test_list1, pTrack[1])) 
			result++;
		if(pTrack[0] != list_get_num(test_list1, 4)) 
			result++;
		if(pTrack[1] != list_get_num(test_list1, 5)) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Linked List - Create Data Array - Pointer Tracking");
		...
		pArr[5] = &test_list1;
		if(list_data_array(test_list1, pArr, 6) != 0) 
			result++;
		if(pArr[0] != pTrack[0]) 
			result++;
		if(pArr[1] != pTrack[1]) 
	...
	test_msg_start("Test Linked List - Create Node Array - Pointer Tracking");
		...
		pTrack[4] = list_ins_tail(test_list1);
		pArr[5] = &test_list1;
		if(list_node_array(test_list1, pArr, 6) != 0) 
			result++;
		if(pArr[0] != pTrack[0]) 
			result++;
		if(pArr[1] != pTrack[1]) 
	...
	test_msg_start("Test Linked List - Search List");
		list_clear(test_list1);
		if(list_search(test_list1, test_node1) != 0) 
			result++;
		test_node1 = list_ins_head(test_list1);
		if(list_search(test_list1, test_node1) != 1) 
			result++;
		if(list_search(test_list1, test_node2) != 0) 
			result++;
		if(list_search(test_list1, test_node3) != 0) 
			result++;
		if(list_search(test_list1, NULL) != 0) 
			result++;
		if(list_search(test_list1, (List_Node *)test_list1) != 0) 
			result++;
		test_node2 = list_ins_head(test_list1);
		if(list_search(test_list1, test_node1) != 1) 
			result++;
		if(list_search(test_list1, test_node2) != 1) 
			result++;
		if(list_search(test_list1, test_node3) != 0) 
			result++;
		if(list_search(test_list1, NULL) != 0) 
...
int test_vstack() {
	...
	test_msg_start("Test Variable Stack - Popping Data Off Empty Stack");
		...
		if(vstack_pop(test_stack) != -1) 
			result++;
	test_msg_end(result);

	test_msg_start("Test Variable Stack - Tracking Stack Size");
		vstack_clear(test_stack);
		if(vstack_size(test_stack) != 0) 
			result++;
		vstack_push(test_stack);
		if(vstack_size(test_stack) != 1) 
			result++;
		temp_node1 = vstack_peek(test_stack);
		if(vstack_size(test_stack) != 1) 
			result++;
	...
	test_msg_start("Test Variable Stack - Tracking Node Address With Peek and Push");
		...
		if(vstack_peek(test_stack) != temp_node2) 
			result++;
		vstack_pop(test_stack);
		if(vstack_peek(test_stack) != temp_node1) 
			result++;
	test_msg_end(result);
...
int test_vqueue() {
	...
	test_msg_start("Test Queue - Tracking Queue Size");
		vq_clear(test_q1);
		if(vq_size(test_q1) != 0) 
			result++;
		vq_enq(test_q1);
		if(vq_size(test_q1) != 1) 
			result++;
		test_node1 = vq_peek(test_q1);
		if(vq_size(test_q1) != 1) 
			result++;
	...
	test_msg_start("Test Queue - Tracking Node Address With Enqueue and Dequeue");
		...
		if(vq_peek(test_q1) != test_node1) 
			result++;
		vq_deq(test_q1);
		if(vq_peek(test_q1) != test_node2) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Queue - Copying Queue");
		...
		test_node1->pData = &result;
		test_node2 = vq_enq(test_q1);
		if(vq_copy(test_q2, test_q1) != 0) 
			result++;
		if(test_q1->pNext == test_q2->pNext) 
			result++;
		if(test_q1->count != 2) 
			result++;
		if(test_q2->count != 2) 
			result++;
		if(test_q1->pNext->pData != test_q2->pNext->pData) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Queue - Copying Empty Queue");
		...
		if(vq_copy(test_q2, test_q1) != 0) 
			result++;
		if(test_q1->pNext != NULL) 
			result++;
		if(test_q2->pNext != NULL) 
			result++;
		if(test_q1->count != 0) 
			result++;
		if(test_q2->count != 0) 
	...
	test_msg_start("Test Queue - Swap Nodes In List");
		...
		pTrack[1] = test_node1->pNext;
		pTrack[2] = test_node2->pNext;
		if(vq_node_swap(test_node1, test_node2) != 0) 
			result++;
		if(test_node1->pNext != pTrack[2]) 
			result++;
		if(test_node2->pNext != pTrack[1]) 
...
int test_hash() {
	...
	test_msg_start("Test Hash - Twin Prime Number Generator");
		if(next_twinprime(6) != 7) 
			result++;
		if(next_twinprime(72) != 73) 
			result++;
		if(next_twinprime(1000) != 1021) 
			result++;
		if(next_twinprime(79154) != 79231) 
			result++;
		if(next_twinprime(INT_MAX) != -1) 
			result++;
	test_msg_end(result);
	...
	test_msg_start("Test Hash - Insert and Verify Data");
		...
		if(hash_search(h, "hello", 6) == NULL) 
			result++;
		if(hash_search(h, "goodbye", 7) != NULL) 
			result++;
		if(hash_search(h, "", 0) != NULL) 
			result++;
	test_msg_end(result);
